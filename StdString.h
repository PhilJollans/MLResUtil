...
// of the Standard C++ Library, you can define your own version of SS_USE_FACET.
  ...
  // for a list of microsoft compiler version numbers.
...
// Promote them to the global namespace so we can use them here.
	...
	{
        // JP changed from _vsnprintf to _vsnprintf_s , so that _CRT_SECURE_NO_WARNINGS is not required.
#if _MSC_VER > 1300
		return _vsnprintf_s ( pA, nCount+1, nCount, pFmtA, vl ) ;
#else
	...
	inline int	ssnprintf(PWSTR pW, size_t nCount, PCWSTR pFmtW, va_list vl)
	{
        // JP changed from _vsnprintf to _vsnprintf_s , so that _CRT_SECURE_NO_WARNINGS is not required.
#if _MSC_VER > 1300
		return _vsnwprintf_s ( pW, nCount+1, nCount, pFmtW, vl ) ;
#else
...
//		}
	...
	#define FMT_BLOCK_SIZE		128  // # of bytes to increment per try
